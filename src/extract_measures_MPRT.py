...
def compute_measures_for_sample(network:torch.nn.Module,\
        ...
        all_measures['EfficientMPRT'] = np.zeros(num_rankings, dtype=np.float32)
        all_measures['EfficientMPRT_inv'] = np.zeros(num_rankings, dtype=np.float32)
        ...
        for i,f in enumerate(attribution_functions):
            ...
            #print(a_batch)
            all_measures['EfficientMPRT'][i] = emprt_function(model=network,
                                                                x_batch=x_batch, 
                                                                y_batch=y_batch,
                                                                a_batch=a_batch,
...
if __name__ == '__main__':
    ...
    for DATASET, MODEL_NAME in tuples_to_test:
        ...
        for sample_index in SAMPLE_INDICES:
            ...
            quantus_results = {}
            quantus_results['EfficientMPRT'] = all_measures['EfficientMPRT']
            quantus_results['EfficientMPRT_inv'] = all_measures['EfficientMPRT_inv']

            np.savez(os.path.join(PROJ_DIR, 'results', f'{DATASET}_{sample_index}_{MODEL_NAME}_EfficientMPRT_measures.npz'), \
