...
def _repopulate(population:np.ndarray[int], num_saved:int, random_elements:int = 0) -> np.ndarray[int]:
    '''
    ...
    '''
    rng = default_rng()
    for i in range(num_saved, population.shape[0] - random_elements): # Loop through all non-top performers to overwrite them
    ...
    for i in range(random_elements):
        population[-random_elements + i] = rng.permutation(population.shape[1])
...
def generate_rankings(num_rankings: int,\
                      ...
                      new_random_elements_per_iteration:int =0,\
    ...
    population = np.zeros((population_size + new_random_elements_per_iteration, num_features), dtype=int)
    rng = default_rng()
    ...
    for i in range(num_iterations):
        ...
        if i < num_iterations - 1: # Don't do this in the last iteration
            _repopulate(population, num_rankings, new_random_elements_per_iteration)
